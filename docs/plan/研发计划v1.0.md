...
| **P1** | 接口协议与数据库设计 | OpenAPI规范 + DDL脚本 | 1天 | G1 |
| **P2** | 核心基础设施开发 | VLM客户端 + 知识库加载器 + 提示词框架 | 2.5天 | G2 |
| **P3** | 诊断引擎核心逻辑 | Q0-Q6问诊 + 三层渐进诊断 | 2天 | G3 |
| **P4** | 诊断API开发 | FastAPI服务 + 图片存储 | 1.5天 | G4 |
...
- **D+1.5（G1通过）**：数据库表创建完成，API接口规范评审通过
- **D+4（G2通过）**：VLM调用成功，知识库加载验证通过
- **D+6（G3通过）**：完整诊断引擎逻辑验证通过（单疾病测试）
- **D+7.5（G4通过）**：诊断API运行，Postman测试通过
- **D+9.5（G5通过）**：管理后台运行，疾病CRUD功能验证
- **D+11（G6通过）**：Web界面运行，图片上传诊断成功
- **D+13（G7通过）**：所有测试用例通过
...
**目标**：实现VLM客户端、知识库加载器、提示词框架、模糊匹配引擎、加权诊断评分器
...
**目标**：实现Q0-Q6问诊序列、三层渐进诊断流程、VLM兜底策略
...
产出物清单：
- [ ] `backend/services/diagnosis_service.py`（完整实现）
- [ ] `backend/tests/integration/test_q0_filtering.py`
- [ ] `backend/tests/integration/test_diagnosis_flow.py`
...
**目标**：实现FastAPI服务、诊断路由、图片存储服务、简单认证中间件
...
2. 按准确率+花卉名+日期分类存储：
   - `storage/images/unlabeled/{genus}/{year-month}/{day}/`
   - 文件命名：`{diagnosis_id}_{disease_id}.jpg`
...
产出物清单：
- [ ] `backend/apps/api/` 下所有文件
- [ ] `backend/infrastructure/storage/` 下所有文件
- [ ] `backend/services/image_service.py`
...
| G2 | P2 | PROOF Framework + VLM客户端 + 知识库加载器 + 模糊匹配 + 评分器 |
| G3 | P3 | Q0过滤 + Q1-Q6特征提取 + 三层诊断 + VLM兜底 |
| G4 | P4 | FastAPI启动 + 诊断API测试 + 图片存储 + 认证中间件 |
