...
3. **目录结构更新** (第4章): 新增前端目录结构（React完整项目结构）
4. **前端架构设计** (新增第15章): React技术选型、项目初始化、4个界面设计
...
graph TB
    subgraph API["API层 (FastAPI)"]
        DiagnosisRouter["DiagnosisRouter<br/>诊断路由"]
        KnowledgeRouter["KnowledgeRouter<br/>知识库路由"]
    end

    subgraph Services["应用服务层 (Application Services)"]
        DiagnosisService["DiagnosisService<br/>诊断服务"]
        KnowledgeService["KnowledgeService<br/>知识库服务"]
        ImageService["ImageService<br/>图片服务"]
    end

    subgraph Infrastructure["基础设施层 (Infrastructure Modules)"]
        VLMClient["VLMClient<br/>VLM客户端"]
        KnowledgeLoader["KnowledgeLoader<br/>知识库加载器"]
        DiagnosisScorer["DiagnosisScorer<br/>加权诊断评分器"]
        FuzzyMatcher["FuzzyMatcher<br/>模糊匹配引擎"]
        PROOFFramework["PROOF Framework<br/>提示词框架"]
        LocalImageStorage["LocalImageStorage<br/>本地图片存储"]
    end
...
| 版本 | 日期 | 修订内容 | 作者 |
|-----|------|---------|------|
| **v2.0** | 2025-11-14 | 🔄 **重大更新**：<br/>1. 删除第11章"多租户与权限设计"(项目需求中不需要)<br/>2. 扩展第6章API设计，新增知识库管理API(5个接口)、本体管理API(2个接口)<br/>3. 更新第4章目录结构，新增前端完整目录结构<br/>4. 新增第15章"前端架构设计"(React技术选型、4个界面设计)<br/>5. 新增第16章"前后端联调指南"(CORS配置、API测试、E2E测试)<br/>6. 新增第17章"文档修订记录"<br/>7. 章节重新编号：原12-15章重新编号为11-14章 | 系统架构师 |
| **v1.1** | 2025-11-11 21:27:44 | 重构第5章：明确区分应用服务层和基础设施模块层，补充分层架构图和调用关系矩阵，详细定义每个服务/模块的职责、依赖关系和关键接口 | 系统架构师 |
...
**第4章 目录结构扩展**:
- **新增**: 4.2节"前端目录结构"
- **内容**: 完整的React项目目录结构(src/api, src/components, src/pages, src/store, src/types等)
