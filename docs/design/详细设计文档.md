...
class DiagnosisService:
    ...
    async def diagnose(self, image_bytes: bytes, metadata: dict = None) -> DiagnosisResult:
        ...
        if not candidate_diseases:
            return DiagnosisResult(
                ...
                level="unknown",
                confidence=0.0,
                message=f"知识库中暂无 {flower_genus} 的疾病数据",
        ...
        # 【兜底逻辑2】所有候选疾病分数过低（知识库外疾病）
        if not best_match or best_match[1].total_score < 0.30:
            try:
                vlm_diagnosis = await self._vlm_open_ended_diagnosis(image_bytes)
                return DiagnosisResult(
                    diagnosis_id=diagnosis.diagnosis_id,
                    level="vlm_fallback",
                    confidence=0.0,
                    message="知识库未匹配到已知疾病",
                    vlm_suggestion=vlm_diagnosis,
                ...
                return DiagnosisResult(
                    ...
                    confidence=0.0,
                    message="诊断系统暂时不可用",
                    suggestion=f"VLM服务异常: {str(e)}，请稍后重试",
                    feature_vector=feature_vector.to_dict()
                )

        # 中低置信度（0.60 ≤ score < 0.85）：返回疑似诊断
    ...
    async def _vlm_open_ended_diagnosis(self, image_bytes: bytes) -> str:
        ...
        使用场景：
        1. 知识库无候选疾病（该花卉未收录）
        2. 所有候选疾病分数 < 0.30（知识库外疾病）

        返回：VLM的自由文本诊断建议
        """
        open_ended_prompt = """
You are a plant pathology expert. Analyze this flower image and provide disease diagnosis.

TASK: Describe the abnormality and suggest possible diseases.

RESPONSE FORMAT (JSON):
{
  "diagnosis": "Brief diagnosis in Chinese (50-100 characters)",
  "confidence": 0.0,  // Always 0.0 for open-ended diagnosis
  "reasoning": "Detailed observation and reasoning in Chinese (100-200 characters)"
}

IMPORTANT:
- Focus on observable symptoms (leaf spots, wilting, discoloration, etc.)
- Mention possible disease categories (fungal, bacterial, viral, nutrient deficiency)
- Suggest next steps (consult expert, send sample to lab, etc.)
"""

        try:
            raw_response = await self.vlm_client.call_with_fallback(
                prompt=open_ended_prompt,
                image_bytes=image_bytes,
                question_id="vlm_fallback"
            )

            # 解析VLM响应
            data = json.loads(raw_response)
            return data.get("diagnosis", "VLM无法识别，建议人工诊断")

        except Exception as e:
            # VLM完全失败时抛出异常，由上层捕获
            raise VLMError(f"VLM开放式诊断失败: {str(e)}")
```
...
class DiagnosisScorer:
    ...
    def calculate_score(
        ...
        # 计算主要特征匹配情况
        major_matched = self._count_major_matched(observed_features, major_features)
        major_total = len(major_features.get("features", []))
...
**设计目标**：
1. 确保4个VLM Provider（Qwen/ChatGPT/Grok/Claude）输出格式一致
2. 固化FlowerSpecialist方法论v5.0的实验经验（视觉化描述5大方法）
3. 提供完整的Q0-Q6问题模板，开发者可直接复制到代码中实现
4. 提供响应验证机制，处理VLM返回的非标准格式

---

#### 5.6.1 VLM响应格式协议（JSON Schema）

所有VLM Provider必须返回严格符合此协议的JSON字符串：
...
from core.exceptions import VLMProtocolError

class ResponseValidator:
    """VLM响应验证器"""

    # Question ID 到 Schema 的映射
    SCHEMA_MAP: Dict[str, Type[VLMResponse]] = {
        "Q0.0": Q00Response,
        "Q0.1": Q01Response,
        "Q0.2": Q02Response,
        "Q0.3": Q03Response,
    ...
    def validate_and_parse(
        ...
        try:
            ...
            # VLM返回了自然语言，尝试提取
            return cls._extract_from_natural_language(raw_response, question_id)

        except ValidationError as e:
            raise VLMProtocolError(f"VLM响应格式错误: {e}")
    ...
    def _extract_from_natural_language(
        ...
        # 常见模式匹配
        patterns = [
            r'"choice"[:\s]*"([^"]+)"',     # "choice": "Rosa"
            r'choice[:\s]+([a-zA-Z_]+)',     # choice: Rosa
            r'Answer[:\s]+([A-Z])',          # Answer: A
            r'^([A-Z])\.',                   # A. Rosa
            r'I choose ([a-zA-Z_]+)',        # I choose Rosa
        ]
...
**设计原则**：
1. **强制JSON输出**：在System Prompt中明确要求返回JSON格式
2. **融入视觉化描述**：应用方法论v5.0的5大视觉化方法
3. **Few-shot示例**：提供1-2个典型示例，提高VLM理解准确率
4. **错误处理**：允许VLM返回"uncertain"或"unknown"

```python
# infrastructure/llm/prompts/q0_precheck.py
"""
Q0逐级过滤提示词模板
遵循FlowerSpecialist方法论v5.0
"""

Q0_0_CONTENT_TYPE_PROMPT = """
You are a plant disease diagnosis assistant. Analyze this image and identify the main content type.

TASK: Determine what is in the image.

CHOICES:
- plant: 植物（包括花卉、树木、草本等）
- animal: 动物
- person: 人物
- object: 物体（工具、容器等）
- landscape: 风景（山水、建筑等）
...
RESPONSE FORMAT (JSON only):
{
  "choice": "plant",
  "confidence": 0.95,
  "reasoning": "The image shows a flower with leaves"
}

IMPORTANT: Only return JSON, no additional text.
"""

Q0_1_PLANT_CATEGORY_PROMPT = """
You are a plant disease diagnosis assistant. The image contains a plant. Identify the plant category.

TASK: Determine the plant category.

CHOICES:
- flower: 观赏花卉（如玫瑰、郁金香、康乃馨等）
- vegetable: 蔬菜（如番茄、黄瓜、白菜等）
- tree: 树木（如松树、柳树等，不含果树）
- crop: 农作物（如小麦、水稻、玉米等）
- grass: 草坪/草本植物
- other: 其他植物
...
RESPONSE FORMAT (JSON only):
{
  "choice": "flower",
  "confidence": 0.90,
  "reasoning": "The plant has ornamental flowers"
}
...
You are a plant disease diagnosis assistant. The image contains an ornamental flower.

TASK: Identify the genus (属) of this flower.

VISUAL CLUES (方法论v5.0 - 视觉化描述):
...
- Tulipa (郁金香): Long narrow leaves, cup-shaped flowers, smooth stem
- Dianthus (康乃馨): Narrow linear leaves, fringed petal edges, swollen stem nodes
- Paeonia (牡丹): Large compound leaves, large multi-layered flowers, thick stems
...
TASK: Identify which part of the flower is shown in the image.
...
RESPONSE FORMAT (JSON only):
{
  "choice": "leaf",
  "confidence": 0.98,
  "reasoning": "The image clearly shows leaf blades"
}
...
TASK: Determine if the organ is complete or partially visible in the image.
...
VISUAL GUIDE:
- complete: Can see the entire leaf from tip to base, or entire flower
- partial: Some edges are cut off by image boundary
- close_up: Zoomed in on a specific spot/symptom, cannot see full organ

RESPONSE FORMAT (JSON only):
{
  "choice": "partial",
  "confidence": 0.88,
  "reasoning": "The leaf tip is cut off by the image boundary"
}
...
TASK: Determine if there are any visible abnormalities on the plant organ.

CHOICES:
- healthy: 健康（叶色正常、无斑点、无变形、无覆盖物）
- abnormal: 有异常（存在斑点、变色、覆盖物、卷曲、萎蔫、破损等）

NORMAL APPEARANCE:
- Green color (may vary by species: light green to dark green)
- Smooth surface, no spots or discoloration
- No powdery/fuzzy coatings
- No curling, wilting, or distortion

ABNORMAL SIGNS:
- Spots (black, brown, yellow, white等)
- Color changes (yellowing, browning, purpling)
- Coatings (powdery, fuzzy, mold-like)
- Deformations (curling, wilting, holes, necrosis)

RESPONSE FORMAT (JSON only):
{
  "choice": "abnormal",
  "confidence": 0.95,
  "reasoning": "Observed black spots with yellow halos on leaf surface"
}
...
```python
# infrastructure/llm/prompts/q1_q6_features.py
"""
Q1-Q6动态特征提取提示词模板
根据Q0.5返回的symptom_type动态生成
"""

Q1_SYMPTOM_TYPE_PROMPT = """
You are a plant disease diagnosis assistant. The plant shows abnormalities.

TASK: Identify the PRIMARY symptom type.

CHOICES:
- necrosis_spot: 坏死性斑点（黑色、褐色死亡组织）
- powdery_coating: 白粉覆盖（粉状白色覆盖物）
- chlorosis: 褪绿/黄化（叶片失绿变黄）
- rust_pustule: 锈斑（橙色/褐色隆起脓疱）
- mosaic_pattern: 花叶/斑驳（绿色深浅不一的斑驳）
- wilting: 萎蔫（叶片下垂、失水）
- leaf_curling: 卷叶（叶片向上或向下卷曲）
- other: 其他症状
...
RESPONSE FORMAT (JSON only):
{
  ...
  "confidence": 0.92,
  "reasoning": "Observed dark spots with dead tissue"
}

IMPORTANT: Only return JSON, no additional text.
"""

# Q2-Q6: 根据symptom_type动态生成
# 示例：symptom_type = "necrosis_spot"
Q2_COLOR_CENTER_PROMPT_NECROSIS = """
You are a plant disease diagnosis assistant. The symptom type is: necrosis_spot (坏死性斑点).

TASK: Identify the COLOR of the SPOT CENTER (not the border).
...
FEW-SHOT EXAMPLE:
Input: Spot center looks like burnt paper, very dark
Output: {
  "choice": "black",
  "confidence": 0.90,
  "reasoning": "The center is very dark, resembling burnt tissue"
}
...
Q3_COLOR_BORDER_PROMPT_NECROSIS = """
You are a plant disease diagnosis assistant. The symptom type is: necrosis_spot.

TASK: Check if there is a COLORED HALO (晕圈) around the spot.

VISUAL METHOD (方法论v5.0 - 3-Zone Comparison):
Compare three zones:
- Zone A (spot center): The dark necrotic area
- Zone B (border, 1-3mm around center): Is there a distinct colored ring? 像煎蛋的蛋白环绕蛋黄
- Zone C (normal tissue): Green healthy tissue

CHOICES:
- yellow: 黄色晕圈 (like egg white around yolk)
- light_yellow: 浅黄色晕圈
- no_halo: 无晕圈 (spot edges directly meet green tissue)
- unclear: 不清楚 (image quality insufficient)

FEW-SHOT EXAMPLE:
Input: Around the dark spot, there is a distinct yellow ring (1-2mm wide) before the green tissue
Output: {
  "choice": "yellow",
  "confidence": 0.95,
  "reasoning": "Clear yellow halo observed between spot and healthy tissue, like egg white pattern"
}
...
# Q4-Q6: location, size, distribution
# (类似模板，此处省略完整代码)
```
...
├── __init__.py
├── q0_precheck.py          # Q0.0-Q0.5固定模板
├── q1_q6_features.py       # Q1-Q6动态模板
├── response_schema.py      # Pydantic响应模型
...
async def test_prompt_version(
    ...
    """对比新旧提示词的准确率"""
    old_accuracy = await run_diagnosis_batch(test_images, old_prompts)
    new_accuracy = await run_diagnosis_batch(test_images, new_prompts)

    return {
        "old_version": old_accuracy,
        "new_version": new_accuracy,
...
class VLMProvider(ABC):
    ...
    async def call_validated(
        ...
        """调用VLM并验证响应格式"""
        raw_response = await self.call(prompt, image)
        validated = ResponseValidator.validate_and_parse(
            raw_response,
            question_id
        )
        return validated
...
**Provider实现示例**：
```python
# infrastructure/llm/providers/qwen.py
class QwenVLPlusProvider(VLMProvider):
    async def call(self, prompt: str, image: bytes) -> str:
        """调用Qwen VL Plus API"""
        # System prompt强制JSON输出
        system_prompt = (
