...
【Layer 3】置信度分层决策
  ...
  │   └─ 触发二次问诊机制 (follow-up questions)
  │       ├─ 从qa_pairs.json加载鉴别问题（1-2个）
  │       ├─ 向用户提问关键特征（区分相似疾病）
  │       ├─ 用户回答后重新计算特征分数
  │       └─ 更新诊断结果 (confirmed / suspected)
  │
...
- **0.60-0.85（疑似）**: 部分特征匹配，需二次问诊提升置信度
...
#### 二次问诊机制（Follow-up Questions）

**触发条件**：
- 诊断置信度在 **0.60 ≤ score < 0.85** 范围内
- 即Layer 3判定为"Medium Confidence"时自动触发
...
```json
{
  "differential_diagnosis": {
    "rose_black_spot_vs_downy_mildew": {
      "description": "区分玫瑰黑斑病和霜霉病",
      "candidate_diseases": ["rose_black_spot", "rose_downy_mildew"],
      "questions": [
        {
          "question_id": "Q_RBS_RDM_1",
          "question": "叶片背面是否有灰白色霉层？",
          "question_en": "Is there a grayish-white mold layer on the underside of the leaf?",
          "options": [
            {"value": "yes", "score_adjustments": {"rose_downy_mildew": +0.15, "rose_black_spot": -0.10}},
            {"value": "no", "score_adjustments": {"rose_black_spot": +0.10, "rose_downy_mildew": -0.15}},
            {"value": "unsure", "score_adjustments": {}}
          ],
          "rationale": "霜霉病特征：叶背灰白色霉层；黑斑病无此特征"
        },
        {
          "question_id": "Q_RBS_RDM_2",
          "question": "斑点边缘是否有明显的黄色晕圈？",
          "question_en": "Is there a distinct yellow halo around the spot edges?",
          "options": [
            {"value": "yes", "score_adjustments": {"rose_black_spot": +0.15}},
            {"value": "no", "score_adjustments": {"rose_black_spot": -0.10}},
            {"value": "unsure", "score_adjustments": {}}
          ],
          "rationale": "黑斑病关键特征：黄色晕圈（yellow halo）"
        }
      ]
    },
    "cherry_powdery_mildew_vs_downy_mildew": {
      "description": "区分樱花白粉病和霜霉病",
      "candidate_diseases": ["cherry_powdery_mildew", "cherry_downy_mildew"],
      "questions": [
        {
          "question_id": "Q_CPM_CDM_1",
          "question": "白色覆盖物是否呈粉末状，容易擦掉？",
          "question_en": "Is the white coating powdery and easily wiped off?",
          "options": [
            {"value": "yes", "score_adjustments": {"cherry_powdery_mildew": +0.20}},
            {"value": "no", "score_adjustments": {"cherry_downy_mildew": +0.15}}
          ],
          "rationale": "白粉病特征：粉末状易擦；霜霉病较牢固"
        }
      ]
    }
  }
...
```
1. Layer 2 诊断完成，计算total_score = 0.73（疑似）
   ↓
2. 识别Top 2候选疾病：rose_black_spot (0.73), rose_downy_mildew (0.65)
   ↓
3. 查询qa_pairs.json → 找到对应的differential_diagnosis组
   ↓
4. 向用户展示1-2个鉴别问题
   ↓
5. 用户回答（例如：Q_RBS_RDM_1选"no"，Q_RBS_RDM_2选"yes"）
   ↓
6. 应用score_adjustments：
   - rose_black_spot: 0.73 + 0.10 + 0.15 = 0.98 → confirmed
   - rose_downy_mildew: 0.65 - 0.15 = 0.50 → unlikely
   ↓
7. 返回更新后的诊断结果（置信度提升）
```

**实现要点**：
- **问题选择策略**：优先提问score差距最小的Top 2疾病
- **问题数量**：最多2个（避免用户疲劳）
- **可选答案**：支持"unsure"（用户不确定时不调整分数）
- **记录追踪**：记录二次问诊的问题和答案，用于后续分析
    ...
    "questions_asked": [
      {
        ...
        "question": "叶片背面是否有灰白色霉层？",
        "user_answer": "no",
        "score_impact": {"rose_black_spot": +0.10, "rose_downy_mildew": -0.15}
      }
    ],
...
- **存储格式**：JSON文件（易于编辑和版本控制）
  ...
  - `knowledge_base/qa_pairs.json`（二次问诊鉴别问题）
...
**问答对固化方式**：
- **Q0-Q6问题序列**：代码内hardcode（逻辑固定，不需要灵活配置）
  - 定义在 `diagnosis_service.py` 的常量或配置文件中
- **二次问诊鉴别问题**：`qa_pairs.json`（需要灵活扩展）
  - 每添加新疾病，可能需要增加鉴别问题
  - JSON格式便于管理后台编辑和LLM辅助生成
...
| v1.3 | 2025-11-10 | **三层渐进诊断 + 二次问诊 + 实现细节**：<br>**【核心架构】**<br>1. 新增三层渐进诊断流程（Layer1 VLM提取 → Layer2 知识库匹配 → Layer3 置信度分层决策）<br>2. 置信度阈值明确：≥0.85直接报告，0.60-0.85触发二次问诊，<0.60兜底/认怂<br><br>**【二次问诊机制】**<br>3. 完整设计：触发条件、qa_pairs.json格式、鉴别问题示例、流程说明<br>4. 输出结构增加follow_up字段（记录二次问诊过程和置信度变化）<br><br>**【知识库范围】**<br>5. 明确5种花卉列表：Rosa/Prunus/Tulipa/Dianthus/Paeonia<br>6. 明确18-24种疾病列表（每种花卉3-6种疾病）<br>7. v1.2扩展目标：数据集花卉（Hibiscus/Crape Jasmine等）<br><br>**【知识库扩展】**<br>8. 新增知识库扩展流程（新花卉/新疾病添加步骤，预估成本30-50h/花卉）<br>9. 质量保障机制（≥65%准确率、版本控制、回滚机制）<br><br>**【技术实现】**<br>10. 本体实现方式：JSON存储 + Pydantic类封装<br>11. 问答对固化：Q0-Q6 hardcode，二次问诊qa_pairs.json<br>12. 新增F6简单Web验证界面（上传图片+显示结果）<br>13. 验收标准新增：二次问诊验收、Web界面验收 | Claude |
